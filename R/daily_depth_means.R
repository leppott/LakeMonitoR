...
#'
#' @param data data frame of site id, data/time, depth, and measurement
#' (e.g., temperature).
...
#'
#' @export
daily_depth_means <- function(data
                              , col_siteid
                              , col_datetime
                              , col_depth
                              , col_measure){
 ...
 # xts_index <- utils::getFromNamespace("index", "xts")
  ...
  data <- data.frame(data[, c(col_siteid
                              , col_datetime
                              , col_depth
                              , col_measure)])

  # na.omit, just in case
  data <- stats::na.omit(data[, c(col_siteid
                                  , col_datetime
                                  , col_depth
                                  , col_measure)])

  #Find the maximum and minimum depths in the dataset and round them to the
  #  nearest whole number
  #Create a list with each whole number in between those two numbers
  #  (including the max and min)
  ...
  for (i in laketempdepths){
    ...
    #this creates an xts time series. The inputs will be the temperature and
    #   date/time fields
    #as.xts(Temperature,Date_Time)
    depth.xts <- xts::as.xts(depth[, col_measure]
...
#    depthmean <- data.frame(col_fun_date = xts_index(depth.mean.xts)
  ...
  for (j in meandays){
    ...
    if (rowcount > 1){
      interpolated <- stats::approx(Daysubset[, col_fun_depth]
                                    , Daysubset[, col_fun_measure]
                             , method = "linear"
                             , xout = Standard_Depths)
      Standardized.row <- data.frame(col_fun_date = unique(Daysubset[
                                                               , col_fun_date])
      ...
      Standardized.row <- data.frame(col_fun_date = unique(Daysubset[
                                                                , col_fun_date])
                                     , col_fun_depth = round(Daysubset[
                                                   , col_fun_depth], digits = 0)
                                     , col_fun_measure = Daysubset[
                                                             , col_fun_measure])
    }## IF ~ rowcount ~ END
  ...
  # rename columns since can't use variables in data.frame as col names
  names(Standardized.msr.data) <- c(col_fun_date
                                    , col_fun_depth
                                    , col_fun_measure)

  Standardized.msr.data[, col_fun_date] <- as.Date(Standardized.msr.data[
                                                                , col_fun_date])
  Standardized.msr.data <- Standardized.msr.data[order(Standardized.msr.data[
                                                             , col_fun_date]), ]
